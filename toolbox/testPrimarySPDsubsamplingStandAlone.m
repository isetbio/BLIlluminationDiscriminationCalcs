function testPrimarySPDsubsamplingStandAlone
    ...
    for subSamplingFactorIndex = 1:numel(subSamplingFactorsTested)
        ...
        NicePlot.exportFigToPDF(sprintf('subSamplingFactor_%d',subSamplingFactor),hFig,300);
    ...
    % Set fonts for all axes, legends, and titles
    NicePlot.setFontSizes(hFig, 'FontSize', 12); 
    NicePlot.exportFigToPDF('SubSamplingSummary',hFig,300);
end


function plotDisplayCharacteristics(wavelengthSampling, spds, kernelF)  
    % Plot the characteristics of the three primaries
    h = figure(); clf; set(h, 'Position', [300 500 550 810]);
    
    primaryNames = {'red', 'green', 'blue'};
    for primaryIndex = 1:numel(primaryNames)
        % the primary spd
        subplot(3,1,primaryIndex);
        hold on
        plot(wavelengthSampling, spds(:,primaryIndex), 'r-'); 
        plot(wavelengthSampling, spds(:,primaryIndex), 'k.');
        plot(wavelengthSampling, (1+kernelF)/2*max(spds(:)), 'b-');
        set(gca, 'FontSize', 12); 
        set(gca, 'XLim', [wavelengthSampling(1) wavelengthSampling(end)], 'YLim', [0 max(spds(:))]);
        xlabel('wavelength (nm)'); ylabel('energy (watts/sr/nm/m^2)'); axis 'square'
